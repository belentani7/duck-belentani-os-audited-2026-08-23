...
import { createAudioAsset, createDawRender, createEvent, createOpportunity, createWaveformComment, createLedgerEntry, createReleaseKit, createRoyaltySplit, getAudioAssetByKey, listAudioAssets, listDawRenders, listEvents, listLedgerEntries, listNotificationPreferences, listOpportunities, listProjects, listReleaseKits, listRoyaltySplits, listWaveformComments, saveNotificationPreference, updateProject, updateProjectStatus, listLeadSearches, getLeadSearchByTaskUid, setLeadSearchSchedule, createLeadSearch, listLeadRecords, createLeadSource, createLeadRecord, touchLeadSearch, updateLeadRecord } from "./db";
...
export const appRouter = router({
  ...
  workspace: router({
    ...
    getAudioDownloadUrl: protectedProcedure.input(z.object({ storageKey: z.string().min(1).max(500) })).query(async ({ ctx, input }) => {
      const asset = await getAudioAssetByKey(ctx.user.id, input.storageKey);
      if (!asset) throw new TRPCError({ code: "NOT_FOUND", message: "Arquivo não encontrado." });
      return storageGetSignedUrl(input.storageKey);
    }),
