...
vi.mock("@/lib/trpc", () => ({ trpc: { workspace: { notificationPreferences: { useQuery: () => ({ data: [], isLoading: false, refetch: mocks.refetch }) }, saveNotificationPreference: { useMutation: () => ({ isPending: false, mutate: mocks.mutate }) } } } }));
...
describe("NotificationPanel", () => {
  ...
  it("salva a preferência selecionada no tRPC ao clicar", () => {
    ...
    expect(mocks.mutate).toHaveBeenCalledWith({ channel: "telegram", destination: "@duck_prod", enabled: true, eventTypes: ["render", "client", "money"] });
  });
