...
import { nextVersionNumber } from "../../../shared/audio-version";
...
export default function AudioWorkbench({ projectId }: { projectId?: number }) {
  ...
  const handleFile = async (file?: File) => {
    ...
    try {
      ...
      const { decoded, metadata } = await analyzeUploadAudio(bytes, audioContext, nextVersionNumber(assets.data?.length ?? 0));
      ...
      const peakDb = metadata.peakDb;
      const rmsValue = Number(metadata.rms);
      setRms(rmsValue);
      const peaks = JSON.parse(metadata.waveformJson) as number[];
