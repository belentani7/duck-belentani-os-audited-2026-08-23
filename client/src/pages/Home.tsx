...
export default function Home() {
  ...
  const leadQuery = trpc.workspace.leads.useQuery(undefined, { retry: false });
  const searchesQuery = trpc.workspace.leadSearches.useQuery(undefined, { retry: false });
  ...
  const hotLeads = leads.filter((lead) => lead.score >= 70 && lead.status !== "descartado").length;
  const convertedLeads = leads.filter((lead) => lead.status === "convertido").length;
  const searchCount = searchesQuery.data?.length ?? 0;
  const duplicateCount = (searchesQuery.data ?? []).reduce((total, search) => total + search.lastDuplicateCount, 0);
    ...
    <main className="duck-app">
      <aside className={`duck-sidebar ${mobileOpen ? "is-open" : ""}`}>
        ...
        <nav className="main-nav" aria-label="Navegação principal">
          ...
          {navItems.map(({ label, icon: Icon }) => <button key={label} className={`nav-item ${activeNav === label ? "active" : ""}`} onClick={() => { setActiveNav(label); setMobileOpen(false); }}><Icon size={17} /><span>{label}</span>{label === "Oportunidades" && <span className="nav-badge">{hotLeads || leads.length || 0}</span>}</button>)}
          <p className="nav-label secondary-label">Sistema</p>
          <button className={`nav-item ${activeNav === "Automações" ? "active" : ""}`} onClick={() => setActiveNav("Automações")}><Radio size={17} /><span>Automações</span><span className="live-pill">LIVE</span></button>
...
function ProjectRow({ project, playing, onPlay }: { project: { name: string; artist: string; status: string; progress: number; color: string; responsible: string; version: string; time: string }; playing: boolean; onPlay: () => void }) {
  return <div className="project-row"><button className={`play-button ${project.color}`} onClick={onPlay} aria-label={`Reproduzir ${project.name}`}>{playing ? <span className="pause-bars"><i /><i /></span> : <Play size={15} fill="currentColor" />}</button><div className="project-main"><div className="project-title"><strong>{project.name}</strong><span>{project.artist}</span></div><div className="project-progress"><div className="progress-track"><span className={project.color} style={{ width: `${project.progress}%` }} /></div><small>{project.progress}%</small></div></div><div className="project-meta"><span className={`status-tag ${project.color}`}>{project.status}</span><small>{project.version} · {project.responsible}</small></div><time>{project.time}</time><button className="row-more" aria-label="Mais opções"><MoreHorizontal size={18} /></button></div>;
}
